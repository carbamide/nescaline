...
- (void)loadView
{
    ...
    if (UIInterfaceOrientationIsLandscape(orientation) == YES) {
        if ([settings boolForKey:@"fullScreen"] == YES) {
            screenRect = CGRectMake(([settings boolForKey:@"aspectRatio"] == YES) ? 56.0 : 0.0, 0.0,
                                    ([settings boolForKey:@"aspectRatio"] == YES) ? 341.0 : height, 320.0);
        }
        else {
    ...
    [[screenView layer] setShadowColor:[[UIColor blackColor] CGColor]];
    [[screenView layer] setShadowOffset:CGSizeMake(0, 10)];
    [[screenView layer] setShadowOpacity:0.8];
    [[screenView layer] setShadowRadius:15];
    
    [[screenView layer] setShadowPath:[[UIBezierPath bezierPathWithRect:[screenView frame]] CGPath]];
    ...
    if (UIInterfaceOrientationIsLandscape(orientation) == YES) {
        ...
        controllerView = [[ControllerView alloc] initWithFrame:
                          CGRectMake(0.0, height == 568 ? 398 : 310.0, self.view.frame.size.width, 105.0)
           ];
    }
