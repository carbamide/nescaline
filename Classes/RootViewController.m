...
- (void)initializeWaitView
{
    ...
    label = [[UILabel alloc] initWithFrame:CGRectMake(135.0, 177.0, 200.0, 25.0)];
...
- (void)startGame:(BOOL)loadState
{
    ...
    [emulatorCore initializeEmulator] ? NSLog(@"Successfully initialized emulator") : NSLog(@"Failure initializing emulator");
...
- (void)didSelectGameROMAtPath:(NSString *)path
{
    ...
    selectedROMSheet = [[UIActionSheet alloc] initWithFrame:CGRectMake(0.0, 480.0, 320.0, 240.0)];
    ...
    [selectedROMSheet setCancelButtonIndex:2];

    [selectedROMSheet showInView:(UIView *)[window subviews][0]];
}
    ...
    if (actionSheet == selectedROMSheet) {
        ...
        if ([[currentROMImagePath pathExtension] isEqualToString:@"sav"]) {
            if (buttonIndex == 0) {
                ...
                deleteSavedGameSheet = [[UIActionSheet alloc] initWithFrame:CGRectMake(0.0, 480.0, 320.0, 240.0)];
                ...
                [deleteSavedGameSheet showInView:(UIView *)[window subviews][0]];
            }
    ...
    (UIViewController *)viewController animated:(BOOL)animated
{
    ...
    if (emulatorCore != nil) {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"autoSave"] == YES) {
            ...
            [saveStateSheet showInView:(UIView *)[window subviews][0]];
        }
    }
}
